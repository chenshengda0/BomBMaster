...
#定时任务
echo "* * * * * date >> /opt/build/node-server-publish.md 2>&1" >> ~/init-crontab
#echo "* * * * * /opt/one_consumer.sh >> /opt/build/test.md 2>&1" >> ~/init-crontab
echo "* * * * * /opt/three_publish.sh >> /opt/build/node-server-publish.md 2>&1" >> ~/init-crontab
#echo "0 0 * * * /opt/RestartApi.sh >> /opt/build/test.md 2>&1" >> ~/init-crontab
echo "*/5 * * * * /opt/RestartCrontab.sh >> /opt/build/node-server-publish.md 2>&1" >> ~/init-crontab
