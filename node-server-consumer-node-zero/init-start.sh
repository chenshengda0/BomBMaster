...
#定时任务
echo "* * * * * date >> /opt/build/node-server-consumer.md 2>&1" >> ~/init-crontab
echo "* * * * * /opt/one_consumer.sh >> /opt/build/node-server-consumer.md 2>&1" >> ~/init-crontab
...
echo "* * * * * /opt/RestartCrontab.sh >> /opt/build/node-server-consumer.md 2>&1" >> ~/init-crontab
